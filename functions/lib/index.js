...
function buildAdsterraUrl(sessionId, userId) {
...
async function countCompletedAdRewardsSince(userId, sinceMillis) {
...
async function getLastCompletedAdRewardMillis(userId) {
    const sessions = await db.collection('adRewardSessions').where('userId', '==', userId).get();
...
async function validateAdRewardRisk(userId) {
    const [flagged, riskScore] = await Promise.all([
        hasHighRiskFraudFlag(userId),
        getLatestRiskScore(userId),
    ]);
    if (flagged)
        return { ok: false, reason: 'high_risk_fraud_flag' };
    if (riskScore > 70)
        return { ok: false, reason: 'high_risk_score' };
    return { ok: true };
}
...
function maskPaymentDetails(details) {
    ...
    if (accountNumber.length > 4) {
        ...
        if (masked.accountNumber)
            masked.accountNumber = maskedValue;
        if (masked.iban)
            masked.iban = maskedValue;
    }
    ...
    if (walletAddress.length > 12) {
        ...
        if (masked.walletAddress)
            masked.walletAddress = maskedValue;
        if (masked.address)
            masked.address = maskedValue;
    }
    return masked;
}
...
async function getExistingReferralByCode(code) {
...
async function hasHighRiskFraudFlag(userId) {
...
async function hasSharedDevice(referrerId, referredId) {
    ...
    const referrerDeviceIds = new Set();
    const referrerFingerprints = new Set();
    referrerDevices.docs.forEach((doc) => {
        const data = doc.data();
        if (data.deviceId)
            referrerDeviceIds.add(data.deviceId);
        if (data.browserFingerprint)
            referrerFingerprints.add(data.browserFingerprint);
    });
    return referredDevices.docs.some((doc) => {
        const data = doc.data();
        return ((data.deviceId && referrerDeviceIds.has(data.deviceId)) ||
            (data.browserFingerprint && referrerFingerprints.has(data.browserFingerprint)));
    });
}
async function countReferralsSince(referrerId, sinceMillis) {
...
async function validateReferralProtections(referrerId, referredId) {
...
async function countCompletedTasks(userId) {
...
async function isReferralQualified(referralId) {
    const referralSnap = await db.collection('referrals').doc(referralId).get();
    if (!referralSnap.exists)
        return { qualified: false, reason: 'referral_not_found' };
    const referral = referralSnap.data();
    if (referral.status === 'rejected')
        return { qualified: false, reason: referral.rejectionReason || 'rejected' };
    ...
    if (!referredSnap.exists)
        return { qualified: false, reason: 'referred_user_not_found' };
    if (!fraudCheck.valid)
        return { qualified: false, reason: fraudCheck.reason };
...
async function validateReferralProtectionsForExisting(referrerId, referredId) {
    if (referrerId === referredId) {
        return { valid: false, reason: 'self_referral' };
    }
    const [referrerFlagged, referredFlagged, referrerRisk, referredRisk, sharedDevice] = await Promise.all([
...
async function grantReward(params) {
    ...
    return db.runTransaction(async (transaction) => {
        ...
        const currentRewardState = (userData.rewardState || {});
...
export const processRegistrationBonus = onCall({ region: REGION }, async (request) => {
    const uid = requireAuthUid(request);
...
export const processDailyLoginBonus = onCall({ region: REGION }, async (request) => {
    const uid = requireAuthUid(request);
    return grantReward({
        userId: uid,
...
export const getRewardStatus = onCall({ region: REGION }, async (request) => {
    ...
    if (!userSnap.exists)
        throw new HttpsError('not-found', 'User profile was not found.');
    const rewardState = (userSnap.data()?.rewardState || {});
...
export const registerReferral = onCall({ region: REGION }, async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!referredSnap.exists)
            throw new HttpsError('not-found', 'Referred user was not found.');
...
async function grantReferralRewardInternal(referralId) {
    ...
    if (reward.granted) {
        ...
        await db.runTransaction(async (transaction) => {
            ...
            if (!currentReferral.exists)
                return;
            if (currentReferral.data()?.rewardGranted === true)
                return;
...
export const processReferralReward = onCall({ region: REGION }, async (request) => {
    ...
    if (!referralId)
        throw new HttpsError('invalid-argument', 'Referral ID is required.');
    const referralSnap = await db.collection('referrals').doc(referralId).get();
    if (!referralSnap.exists)
        throw new HttpsError('not-found', 'Referral was not found.');
    const referral = referralSnap.data();
...
export const checkReferralQualification = onCall({ region: REGION }, async (request) => {
    ...
    if (!resolvedReferralId && refereeId) {
        ...
        if (!referralSnap.empty)
            resolvedReferralId = referralSnap.docs[0].id;
    }
    if (!resolvedReferralId)
        throw new HttpsError('invalid-argument', 'Referral ID or referred user ID is required.');
...
export const getUserReferrals = onCall({ region: REGION }, async (request) => {
    ...
    const qualified = referrals.filter((ref) => ref.status === 'qualified').length;
    ...
    const totalEarnings = referrals
        .filter((ref) => ref.status === 'qualified')
        .reduce((sum, ref) => sum + Number(ref.referrerReward || REFERRAL_REWARD_POINTS), 0);
    return {
        success: true,
        referrals,
...
export const startAdRewardSession = onCall({ region: REGION }, async (request) => {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (!userSnap.exists)
            throw new HttpsError('not-found', 'User profile was not found.');
        const userData = userSnap.data() || {};
        const rewardState = (userData.rewardState || {});
        ...
        const currentSessionStillActive = Boolean(currentSessionId && currentSessionStartedMillis && nowMillis - currentSessionStartedMillis < AD_REWARD_SESSION_EXPIRY_MS);
...
export const completeAdRewardSession = onCall({ region: REGION }, async (request) => {
    const uid = requireAuthUid(request);
    const sessionId = String(request.data?.sessionId || '').trim();
    if (!sessionId)
        throw new HttpsError('invalid-argument', 'Ad reward session ID is required.');
    ...
    const precheck = await db.runTransaction(async (transaction) => {
        ...
        if (session.status === 'verifying')
            return { ok: false, reason: 'session_already_processing' };
        if (session.status !== 'started')
            return { ok: false, reason: 'invalid_session_status' };
        ...
        if (!startedMillis)
            return { ok: false, reason: 'invalid_session_start' };
        if (nowMillis > expiresMillis)
            return { ok: false, reason: 'ad_reward_session_expired' };
...
export const processPendingReferralRewards = onSchedule({ region: REGION, schedule: 'every 60 minutes', timeZone: 'Etc/UTC' }, async () => {
...
export const createWithdrawalRequest = onCall({ region: REGION }, async (request) => {
    ...
    const paymentDetails = (request.data?.bankDetails || request.data?.paymentDetails || {});
    ...
    if (userSnap.data()?.status !== 'active')
        throw new HttpsError('failed-precondition', 'Only active users can request withdrawals.');
    if (riskFlagged || riskScore > 70)
        throw new HttpsError('failed-precondition', 'Withdrawal denied by risk controls.');
